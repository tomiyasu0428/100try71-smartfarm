...
async def register_user(
    user_in: UserCreate, user_service: UserService = Depends(lambda: UserService())
) -> Any:
    """
    新しいユーザーを登録します
    """
    ...
    try:
        ...
        if existing_user:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST, detail="このメールアドレスは既に使用されています"
            )
        ...
        if not result.data:
            raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="ユーザー登録に失敗しました")
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"ユーザー登録に失敗しました: {str(e)}"
        )
